...
def sierpinksi(bottomLeft, bottomRight, top, recursionDepth):
    ...
    if recursionDepth > 0:
        if recursionDepth == 1:
            fill(0)
            triangle(bottomLeft.midPoint(top).getX(),bottomLeft.midPoint(top).getY(),
                    bottomRight.midPoint(top).getX(), bottomRight.midPoint(top).getY(),
                    bottomRight.midPoint(bottomLeft).getX(),bottomRight.midPoint(bottomLeft).getY())
        else:
            for i in range(int(count)):
                sierpinksi(bottomLeft, bottomRight, top, i)
            sierpinksi(bottomLeft.midPoint(top),bottomRight.midPoint(top),top,recursionDepth-1)
            sierpinksi(bottomLeft, bottomRight.midPoint(bottomLeft),bottomLeft.midPoint(top),recursionDepth-1)
            sierpinksi(bottomRight.midPoint(bottomLeft), bottomRight,bottomRight.midPoint(top), recursionDepth-1)
